package com.github.deadwind4.benchmark.flink.kyuubi;
...
//                    "jdbc:trino://host:port/catalog/schema"
        String url = "jdbc:hive2://192.168.96.177:10009/default";
        try {
...
//        properties.setProperty("SSL", "true");
    ...
    private static void run(LinkedHashMap<String, String> queries, int iterations) {
        ...
        try (Connection conn = getConnection()) {
            conn.createStatement().executeQuery("CREATE CATALOG myhive WITH (\n" +
                    "    'type' = 'hive',\n" +
                    "    'default-database' = 'tpcds_bin_orc_10',\n" +
                    "    'hive-conf-dir' = '/Users/ada/tmp/flink-test/hive3-conf'\n" +
                    ")");
            conn.createStatement().executeQuery("USE CATALOG myhive");
    ...
    private static void printSummary(List<Tuple2<String, Long>> bestArray) {
        ...
        printLine(' ', "|", itemMaxLength, "GeoMean", String.valueOf((Math.pow(product, 1d / bestArray.size()) * 1000)));
        printLine('-', "+", itemMaxLength, "", "");
